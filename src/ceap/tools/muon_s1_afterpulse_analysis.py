...
场景 (RELICS 类探测器, 参数取自 config/settings.yaml):
  - μ 子在液氙中沉积能量 → S1 快闪 PE (能损 × 光产额 × QE×CE)
...
输出:
  ...
  2) 1 μs 窗内随机符合事例率 (含 CEvNS 阈值窗)
  3) 事件率分解: 噪声事例率 (Hz) 与 每 μ 子本底事例数

用法: python -m ceap.tools.muon_s1_afterpulse_analysis [--config FILE]
"""
...
DEFAULT_PDF_FILE = "data/app_pdf_LV2358.npz"
...
def analyze_muon(dt_ns, npe, sc: dict):
...
def coincidence_scan(dt_ns, npe, sc: dict, seed: int):
    """1 μs 滑动窗口扫描 [100ns,5us]: 返回每窗口总 PE 与超阈值标记。"""
    rng = np.random.default_rng(seed)
    # 在 [100ns, 5us-window] 内放置 window 滑动窗口
    t_start = T_WIN_MIN_NS
    t_end = T_WIN_MAX_NS - sc["window_us"] * 1e3
    step = 50.0  # ns 步进
    n_win = int((t_end - t_start) / step)
    win_pe = np.zeros(n_win)
    for i in range(n_win):
        lo = t_start + i * step
        hi = lo + sc["window_us"] * 1e3
        mask = (dt_ns >= lo) & (dt_ns < hi)
        win_pe[i] = npe[mask].sum()
    thr_hit = (win_pe >= sc["thr_min"]) & (win_pe <= sc["thr_max"])
...
def main():
    ...
    for i in range(N_MUONS):
        arr = run_muon_afterpulse(sc, seed=1000 + i)
        dt, npe = arr[:, 0], arr[:, 1]
        s = analyze_muon(dt, npe, sc)
        stats.append(s)
        all_dt.append(s["dt"]); all_npe.append(s["npe"])
        win, hit = coincidence_scan(s["dt"], s["npe"], sc, seed=2000 + i)
    ...
    # ---- 统计汇总 ----
    n_pmt, main_pe_total = sc["n_pmt"], sc["main_pe_total"]
    thr_min, thr_max = sc["thr_min"], sc["thr_max"]
    print("=" * 70)
    print(f"μ子 S1 ({main_pe_total:,} PE/{n_pmt} PMT, APP=5%) afterpulse 在 [100ns, 5us] 内特征")
    print("=" * 70)
    ...
    print(f"\n  --- {sc['window_us']:.0f} μs 窗内随机符合 (步进 50 ns, 每 μ 子扫描窗数 {win_all.size//N_MUONS:.0f}) ---")
    print(f"  全部窗平均 PE     : {win_all.mean():.1f} PE")
    print(f"  全部窗 99% 分位   : {np.percentile(win_all,99):.1f} PE")
    frac_120 = (win_all >= thr_min).mean()
    frac_120_240 = hit_all.mean()
    print(f"  窗内总PE≥{thr_min:.0f} PE   : {frac_120*100:.2f}%")
    print(f"  窗内总PE∈[{thr_min:.0f},{thr_max:.0f}]: {frac_120_240*100:.3f}%  (CEvNS 阈值窗口命中率)")

    # 每 μ 子命中数 (一个 μ 子的 afterpulse 产生的假信号数)
    per_muon = np.array([hit.sum() for hit in np.split(hit_all, N_MUONS)])
    print(f"  每 μ 子假信号数   : {per_muon.mean():.2f} ± {per_muon.std():.2f} ({sc['window_us']:.0f}μs窗∈[{thr_min:.0f},{thr_max:.0f}]PE)")
    ...
    fig.suptitle(f"μ子 S1 ({main_pe_total:,} PE/{n_pmt} PMT, APP=5%) afterpulse 特征", fontsize=14)
    ...
    ax.axvline(thr_min, color="r", ls="--", label=f"阈值 {thr_min:.0f} PE")
    ax.axvline(thr_max, color="r", ls=":", label=f"阈值 {thr_max:.0f} PE")
    ax.set_xscale("log")
    ax.set_xlabel(f"{sc['window_us']:.0f} μs 窗内总 PE")
    ax.set_ylabel("窗数")
    ax.set_title(f"{sc['window_us']:.0f} μs 窗口内随机符合 PE 分布")
    ax.legend()
    ...
    ax.axhline(thr_min, color="r", ls="--", label=f"阈值 {thr_min:.0f} PE")
    ax.axhline(thr_max, color="r", ls=":", label=f"阈值 {thr_max:.0f} PE")
    ax.set_xlabel("Δt (ns after S1)")
    ax.set_ylabel(f"{sc['window_us']:.0f} μs 窗内电荷 (PE)")
    ax.set_yscale("log")
    ax.set_title(f"{sc['window_us']:.0f} μs 窗内电荷 vs 窗口位置")
    ax.legend()
    ...
    result = {
        "scenario": {
            ...
            "main_pe_per_pmt": sc["main_pe_per_pmt"],
            "app": 0.053, "pdf": sc["pdf_file"],
            "analysis_window_ns": [T_WIN_MIN_NS, T_WIN_MAX_NS],
            "coincidence_window_us": sc["window_us"],
        },
        "per_muon": {
